...
contract RNGBlockhash is RNGInterface, Ownable {
  ...
  mapping(uint32 => uint256) internal randomNumbers; //维护了 request_id 和 seed 之间的关系

  /// @dev A list of blocks to be locked at based on past requests mapped by request id
  mapping(uint32 => uint32) internal requestLockBlock; //维护了 request_id 和 block 之间的关系
  ...
  function randomNumber(uint32 requestId) external virtual override returns (uint256 randomNum) {
    require(_isRequestComplete(requestId), "RNGBlockhash/request-incomplete"); //requestRandomNumber之后等待至少2个区块产生才能通过
